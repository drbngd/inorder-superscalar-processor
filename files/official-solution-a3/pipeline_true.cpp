...
void pipe_cycle_ID(Pipeline *p)
{
    /*
        ...
        This is incorrect since we haven't stalled yet, so 7 & 8 should always
        be together.
    */  
    for (unsigned int i = 0; i < PIPE_WIDTH; i++) {
    ...
    */  
    for (unsigned int i = 0; i < PIPE_WIDTH; i++) {
        /*
        ============================{ A3 }============================
        Create index trackers for highest op-id dependency of each type.
        ...
        */  
        unsigned int highest_src1_j = PIPE_WIDTH;
        unsigned int highest_src2_j = PIPE_WIDTH;
        unsigned int highest_cc_j = PIPE_WIDTH;
        ...
        */ 
        for (unsigned int j = 0; j < PIPE_WIDTH; j++) {
                ...
                */ 
                if (has_raw_hazards(p, i, j, MA_LATCH)) {
                    ...
                    printf("\t(STALL) I%lu in P%d's ID stage has a RAW hazard on I%lu in P%d's MA stage! (%s)\n", 
                        p->pipe_latch[ID_LATCH][i].op_id, i,
                        p->pipe_latch[MA_LATCH][j].op_id, j,
                        get_hazard(has_raw_hazards(p, i, j, MA_LATCH)));
                    #endif
                }
            }
            ...
            if (ENABLE_EXE_FWD) {
                /*
                ============================{ A3 }============================
                We only have a load-use hazard if the highest op-id (later in program trace) 
                ...
                stages and track the highest op-id hazard-causing pipeline index for src1, src2, 
                ...
                */
                ...
                if (src1_dependency && 
                    (highest_src1_j == PIPE_WIDTH || p->pipe_latch[EX_LATCH][j].op_id > p->pipe_latch[EX_LATCH][highest_src1_j].op_id)) 
                {
                    highest_src1_j = j;
                }
                ...
                if (src2_dependency && 
                    (highest_src2_j == PIPE_WIDTH || p->pipe_latch[EX_LATCH][j].op_id > p->pipe_latch[EX_LATCH][highest_src2_j].op_id)) 
                {
                    highest_src2_j = j;
                }
                ...
                if (cc_dependency &&
                    (highest_cc_j == PIPE_WIDTH || p->pipe_latch[EX_LATCH][j].op_id > p->pipe_latch[EX_LATCH][highest_cc_j].op_id)) 
                {
                    highest_cc_j = j;
                }
            } else {
                ...
                */ 
                if (has_raw_hazards(p, i, j, EX_LATCH)) {
                    ...
                    printf("\t(STALL) I%lu in P%d's ID stage has a RAW hazard on I%lu in P%d's EX stage! (%s)\n", 
                        p->pipe_latch[ID_LATCH][i].op_id, i,
                        p->pipe_latch[EX_LATCH][j].op_id, j,
                        get_hazard(has_raw_hazards(p, i, j, EX_LATCH)));
                    #endif
                }
            }
            /*
            ...
            */ 
                ...
                has_raw_hazards(p, i, j, ID_LATCH)) 
            {
                ...
                printf("\t(STALL) I%lu in P%d's ID stage has a RAW hazard on I%lu in P%d's ID stage! (%s)\n", 
                        p->pipe_latch[ID_LATCH][i].op_id, i,
                        p->pipe_latch[ID_LATCH][j].op_id, j,
                        get_hazard(has_raw_hazards(p, i, j, ID_LATCH)));
                #endif
            }
        }
        /*
        ...
        on pipeline i's ID, check if any of the highest op-id (latest in the program trace) 
        ...
        */
        if (ENABLE_EXE_FWD) {
            if ((highest_src1_j != PIPE_WIDTH && p->pipe_latch[EX_LATCH][highest_src1_j].trace_rec.op_type == OP_LD) ||
                (highest_src2_j != PIPE_WIDTH && p->pipe_latch[EX_LATCH][highest_src2_j].trace_rec.op_type == OP_LD) ||
                (highest_cc_j != PIPE_WIDTH && p->pipe_latch[EX_LATCH][highest_cc_j].trace_rec.op_type == OP_LD)) {
                #ifdef DEBUG
                if (highest_src1_j != PIPE_WIDTH && p->pipe_latch[EX_LATCH][highest_src1_j].trace_rec.op_type == OP_LD) {
                    printf("\t(STALL) Highest op-id src1 RAW hazard in EX is LD! Stalling I%lu in P%d!\n", 
                        p->pipe_latch[ID_LATCH][i].op_id, i);
                }
                if ((highest_src2_j != PIPE_WIDTH && p->pipe_latch[EX_LATCH][highest_src2_j].trace_rec.op_type == OP_LD)) {
                    printf("\t(STALL) Highest op-id src2 RAW hazard in EX is LD! Stalling I%lu in P%d!\n", 
                        p->pipe_latch[ID_LATCH][i].op_id, i);
                }
                if ((highest_cc_j != PIPE_WIDTH && p->pipe_latch[EX_LATCH][highest_cc_j].trace_rec.op_type == OP_LD)) {
...
#ifdef DEBUG
const char *get_hazard(int x) {
    switch (x) {
        case 4: return "cc dependency";
        case 2: return "src2 dependency";
        default: return "src1 dependency";
    }
}
#endif
